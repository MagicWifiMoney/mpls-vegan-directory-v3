#!/usr/bin/env node
/**
 * Process Apify reviews - Fixed version
 * Usage: node process-apify-reviews-fixed2.js "slug"
 */
...
async function main() {
  ...
  if (!restaurantSlug) {
    console.error('❌ Usage: node process-apify-reviews-fixed2.js "slug"');
    process.exit(1);
  }
...
[{"name":"...","description":"...","category":"..."},...]`;
    ...
    try {
      const geminiResult = await execPromise(
        `gemini -p "${prompt.replace(/"/g, '\\"')}" -o json`,
  ...
  const seoKeywords = {
    dishKeywords: { terms: ['ribs', 'burger', 'wrap', 'wings', 'sandwich', 'pizza', 'tacos', 'salad', 'soup', 'steak', 'chicken', 'tofu', 'tempeh', 'seitan', 'omelette', 'pancakes', 'waffles', 'french toast'], points: 3 },
    locationKeywords: { terms: ['minneapolis', 'twin cities', 'northeast', 'uptown', 'st paul', 'saint paul', 'minnesota', 'mn'], points: 2 },
    veganKeywords: { terms: ['vegan', 'plant-based', 'plant based', 'dairy-free', 'dairy free', 'vegetarian'], points: 1 },
    experienceKeywords: { terms: ['handcrafted', 'house-made', 'homemade', 'fresh', 'organic', 'local', 'scratch'], points: 1 },
    useCaseKeywords: { terms: ['meal prep', 'date night', 'lunch', 'dinner', 'brunch', 'breakfast', 'takeout', 'delivery'], points: 1 },
