...
export default function RestaurantCardWithData({ restaurant, index = 0 }: RestaurantCardProps) {
  const { data: placeDetails } = usePlaceDetails(restaurant.googlePlaceId);
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <svg 
                      key={star}
                      className={`w-4 h-4 ${star <= Math.round(rating) ? 'text-[#d4a574]' : 'text-[#f5f0e8]/10'}`}
                      viewBox="0 0 24 24" 
                      fill="currentColor"
                    >
                      <path d="M12 2L9.19 8.63 2 9.24l5.46 4.73L5.82 21 12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2z"/>
                ...
                <span className="text-sm text-[#f5f0e8]/60 font-medium">{rating.toFixed(1)}</span>
              </div>
              {reviewCount && (
                <span className="text-xs text-[#f5f0e8]/30">
